RSpec.describe Event do
  ...
  describe '#has_free_verified_slots?' do
    ...
    before { allow(event).to receive(:limit_total) { 10 } }

    context 'has' do
      before { allow(event).to receive_message_chain(:visitors, :verified, :count) { 4 } }

      it { expect(event.has_free_verified_slots?).to be true }
    end

    context 'not has' do
      before { allow(event).to receive_message_chain(:visitors, :verified, :count) { 12 } }
  ...
  describe '#has_free_newbies_slots?' do
    ...
    context 'has' do
      before { allow(event).to receive_message_chain(:visitors, :newbies, :count) { 3 } }

      it { expect(event.has_free_newbies_slots?).to be true }
    end

    context 'not has' do
      before { allow(event).to receive_message_chain(:visitors, :newbies, :count) { 7 } }
